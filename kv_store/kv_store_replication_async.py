...
class Client(object):
	...
	async def run_op(self):
		...
		if rand_val < 0.5:
			return await self.servers[rand_server].put.remote(str(randrange(self.num_requests)), str(rand_val))
		else:
			return await self.servers[rand_server].get.remote(str(randrange(self.num_requests)))
